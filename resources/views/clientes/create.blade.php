...
@endif
  ...
  @foreach($paises as $pais)
    <option value="{{ $pais->id }}"
      {{ old('pais', $defaultPais) == $pais->id ? 'selected' : '' }}>
      {{ $pais->nombre }}
    </option>
  @endforeach
</select>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.8/js/utils.min.js"></script>
 ...
 @push('scripts')
<script>
console.log("🔥 script de WhatsApp cargado");
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (!iti.isValidNumber()) {
      ...
      fb.textContent = 'Número inválido, por favor corrígelo.';
      return;
    }
  // inyecta el +57… antes de enviar
...
@endpush
  ...
  <!-- Dependent selects de ubicación -->
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      async function cargarDepartamentos() {
        ...
        deptoSelect.innerHTML = '<option value="">Selecciona departamento</option>';
        data.forEach(d => {
          const sel = '{{ old('departamento') }}' == d.id ? 'selected' : '';
          deptoSelect.insertAdjacentHTML('beforeend',
            `<option value="${d.id}" ${sel}>${d.nombre}</option>`);
        });
        cargarMunicipios();
      }
      ...
      async function cargarMunicipios() {
        ...
        muniSelect.innerHTML = '<option value="">Selecciona municipio</option>';
        data.forEach(m => {
          const sel = '{{ old('municipio') }}' == m.id ? 'selected' : '';
          muniSelect.insertAdjacentHTML('beforeend',
            `<option value="${m.id}" ${sel}>${m.nombre}</option>`);
        });
      }
